...
export default function FreelancersPage() {
  ...
  const { data: freelancers, isLoading } = useListFreelancers({ search: q || undefined, skill: skill || undefined, limit, offset: page * limit });
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        ...
        {(q || skill) && (
          <button onClick={() => { setQ(""); setSkill(""); setPage(0); }} className="flex items-center gap-1.5 px-4 py-2 text-sm text-gray-500 border border-gray-200 rounded-xl hover:bg-gray-50">
            <X size={14} /> Clear
          </button>
        )}
      </div>
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="flex items-center justify-center gap-4 mt-10">
            ...
            <button disabled={(freelancers?.length ?? 0) < limit} onClick={() => setPage(p => p + 1)} className="px-5 py-2 border border-gray-200 rounded-xl text-sm font-medium hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed">Next</button>
          </div>
        </>
      ) : (
